...
void combat(Being monster) {
	...
	do {
		...
		do {
			...
			std::cout << ("Monster initiative: " + monsterRoll) << std::endl;
		...
		do {
			std::cout << std::endl;
			std::cout << hero.getCurrentHP() << std::endl;
			std::cout << std::endl;
			std::cout << monster.getCurrentHP() << std::endl;
			std::cout << std::endl;
			if (turn) {
				// hero
				std::cout << std::endl;
				...
				case 3:
					if (hpot == 0) {
						...
						hero.useHealthPot();
						std::cout << (hero.getCurrentHP() + ": current HP.")
								<< std::endl;
					}
					break;
				case 4:
				...
				if ((choice != 3) && (choice != 4)) {
					if (damage > 0) {
						...
						if ((defendRoll == 20) && (attackRoll == 20)) {
							hit = (attackRoll + hero.getStrikeBonus())
				...
				if (damage > 0) {
					...
					std::cout << ("Defender's roll: " + defendRoll)
							<< std::endl;
					if ((defendRoll == 20) && (attackRoll == 20)) {
						hit = (attackRoll + monster.getStrikeBonus())
...
Hero createHero(std::string name) {
	...
	return Hero(stats[0], stats[1], stats[2], name);
...
void rewards() {
	if (hero.isAlive()) {
		...
		std::cout << ("You got " + std::to_string(num) + " potions!")
				<< std::endl;
		...
		std::cout
				<< ("You got " + std::to_string(manas) + " mana pots and "
						+ std::to_string(healths) + " health pots.")
				<< std::endl;
...
void setup() {
	do {
		...
		std::cout
				<< (hero.getName() + ", Strength: "
						+ std::to_string(hero.getStrength()) + ", Dexterity: "
						+ std::to_string(hero.getDexterity())
						+ ", Intelligence: "
						+ std::to_string(hero.getIntelligence())) << std::endl;
	} while (rerollHero());
}
